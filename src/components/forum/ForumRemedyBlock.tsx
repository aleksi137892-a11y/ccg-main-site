import React, { useState } from 'react';
import { motion, AnimatePresence } from 'motion/react';
import { useLanguage } from '@/contexts/LanguageContext';
import { cn } from '@/lib/utils';

interface ForumRemedyBlockProps {
  id: string;
  title: { en: string; ge: string };
  lead: { en: string; ge: string };
  secondary?: { en: string; ge: string };
  activeStates?: { en: string; ge: string };
  jurisdictions?: { en: string[]; ge: string[] };
  externalUrl?: string;
  variant?: 'light' | 'dark';
  className?: string;
}

export function ForumRemedyBlock({
  id,
  title,
  lead,
  secondary,
  activeStates,
  jurisdictions,
  externalUrl,
  variant = 'dark',
  className
}: ForumRemedyBlockProps) {
  const { language } = useLanguage();
  const [isExpanded, setIsExpanded] = useState(false);
  const isDark = variant === 'dark';

  return (
    <div className={cn("space-y-4", className)}>
      {/* Title */}
      <h4 className={cn(
        "font-sans text-sm uppercase tracking-[0.15em]",
        isDark ? "text-white" : "text-navy"
      )}>
        {title[language]}
      </h4>
      
      {/* Lead description */}
      <p className={cn(
        "font-serif text-base leading-relaxed max-w-[55ch]",
        isDark ? "text-white/70" : "text-navy/70"
      )}>
        {lead[language]}
      </p>
      
      {/* Secondary text */}
      {secondary && (
        <p className={cn(
          "font-serif text-sm pl-4 leading-relaxed border-l",
          isDark 
            ? "text-white/50 border-white/10" 
            : "text-navy/55 border-navy/10"
        )}>
          {secondary[language]}
        </p>
      )}
      
      {/* Active states highlight */}
      {activeStates && (
        <p className={cn(
          "font-serif text-sm italic",
          isDark ? "text-white/60" : "text-navy/60"
        )}>
          {activeStates[language]}
        </p>
      )}
      
      {/* Expandable jurisdictions */}
      {jurisdictions && jurisdictions[language].length > 0 && (
        <div>
          <button
            onClick={() => setIsExpanded(!isExpanded)}
            className={cn(
              "inline-flex items-center gap-2 text-sm transition-colors duration-200",
              isDark 
                ? "text-white/50 hover:text-white/80" 
                : "text-navy/50 hover:text-navy/80"
            )}
          >
            <span 
              className="inline-block transition-transform duration-200" 
              style={{ transform: isExpanded ? 'rotate(90deg)' : 'none' }}
            >
              →
            </span>
            <span>{language === 'ge' ? 'იურისდიქციები' : 'Jurisdictions'}</span>
          </button>
          
          <AnimatePresence>
            {isExpanded && (
              <motion.div
                initial={{ height: 0, opacity: 0 }}
                animate={{ height: 'auto', opacity: 1 }}
                exit={{ height: 0, opacity: 0 }}
                transition={{ duration: 0.2, ease: 'easeOut' }}
                className="overflow-hidden"
              >
                <ul className={cn(
                  "pt-4 space-y-2 pl-4 border-l",
                  isDark ? "border-white/10" : "border-navy/10"
                )}>
                  {jurisdictions[language].map((item, index) => (
                    <li 
                      key={index} 
                      className={cn(
                        "font-serif text-sm leading-relaxed",
                        isDark ? "text-white/50" : "text-navy/60"
                      )}
                    >
                      {item}
                    </li>
                  ))}
                </ul>
              </motion.div>
            )}
          </AnimatePresence>
        </div>
      )}
      
      {/* External link if provided (e.g., IIMG) */}
      {externalUrl && (
        <a 
          href={externalUrl}
          target="_blank"
          rel="noopener noreferrer"
          className={cn(
            "inline-flex items-center gap-2 text-sm transition-colors duration-200",
            isDark 
              ? "text-white/60 hover:text-white" 
              : "text-navy/60 hover:text-navy"
          )}
        >
          <span>{language === 'ge' ? 'გარე ბმული' : 'External site'}</span>
          <span>↗</span>
        </a>
      )}
    </div>
  );
}

export default ForumRemedyBlock;
